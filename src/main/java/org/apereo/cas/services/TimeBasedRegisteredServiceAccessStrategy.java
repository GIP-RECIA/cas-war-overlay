...
public class TimeBasedRegisteredServiceAccessStrategy extends BaseRegisteredServiceAccessStrategy {
    ...
    public boolean isServiceAccessAllowed(final RegisteredService registeredService, final Service service) {
        ...
        if(!authorize){
            if(result){
                LOGGER.debug("Service access not allowed because it starts at [{}] and ends at [{}].", startingDateTime, endingDateTime);
            }
            return !result;
        }
        return result;
    ...
    /**
     * Custom enhancement, compare start/end time with current time even after connexion to enable the possibility
     * to chain a TimeBasedRegisteredServiceAccessStrategy with a DefaultRegisteredServiceAccessStrategy
     */
    @Override
    ...
    protected boolean doesEndingTimeAllowServiceAccess() {
        ...
        if (endDateTime != null) {
            ...
            if (et != null) {
                ...
                if (now.isAfter(et)) {
                    if(authorize){
                        LOGGER.debug("Service access not allowed because it ended at [{}]. Now is [{}]", endDateTime, now);
                    }
                    return false;
                }
            } else {
    ...
    protected boolean doesStartingTimeAllowServiceAccess() {
        ...
        if (startDateTime != null) {
            ...
            if (st != null) {
                ...
                if (now.isBefore(st)) {
                    if(authorize) {
                        LOGGER.debug("Service access not allowed because it starts at [{}]. Zoned now is [{}]", startDateTime, now);
                    }
                    return false;
                }
            } else {
                ...
                if (stLocal != null) {
                    ...
                    if (now.isBefore(stLocal)) {
                        if(authorize) {
                            LOGGER.debug("Service access not allowed because it starts at [{}]. Local now is [{}]", startDateTime, now);
                        }
                        return false;
                    }
                }
