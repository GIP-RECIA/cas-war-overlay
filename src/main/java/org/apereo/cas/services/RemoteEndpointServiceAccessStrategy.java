...
public class RemoteEndpointServiceAccessStrategy extends BaseRegisteredServiceAccessStrategy {
    ...
    private RegisteredServiceAccessStrategyActivationCriteria activationCriteria;

    /**
     * Internal DTO to avoid transfer large amounts of data
     */
    private static record RequestPayload(List<Object> ESCOUAICourant, List<Object> isMemberOf) {
        RequestPayload(RegisteredServiceAccessStrategyRequest request) {
            this(request.getAttributes().get("ESCOUAICourant"), request.getAttributes().get("isMemberOf"));
        }
    }

    @Override
    public boolean authorizeRequest(final RegisteredServiceAccessStrategyRequest request) throws Throwable{
        // Customisation : condition request sending by activation criteria
        val proceed = activationCriteria == null || activationCriteria.shouldActivate(request);
        if(proceed){
            return Unchecked.supplier(() -> {
                val payload = new RequestPayload(request);
