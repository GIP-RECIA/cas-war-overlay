...
public class OidcHandlerInterceptorAdapter extends OAuth20HandlerInterceptorAdapter {
    ...
    public boolean preHandle(final HttpServletRequest request, final HttpServletResponse response,
        ...
        if (this.isPushedAuthorizationRequest(request.getRequestURI()) && !this.isValidAuthorizeRequest(new JEEContext(request, response))) {
            LOGGER.warn("Invalid OIDC pushed authorization request at [{}]", (Object)request.getRequestURI());
            response.setStatus(403);
            writeErrorResponseBody((HttpServletResponse)response);
            return false;
        }
    ...
    private void writeErrorResponseBody(final HttpServletResponse response) throws Exception {
