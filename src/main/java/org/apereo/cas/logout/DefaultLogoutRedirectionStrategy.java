            ...
            .ifPresentOrElse(service -> {
                ...
                if (casProperties.getLogout().isFollowServiceRedirects()) {
                    ...
                    if (!registeredService.getProperties().isEmpty()) {
                        if (registeredService.getProperties().containsKey("redirectLogoutURL")){
                            LOGGER.debug("Service is configured to force redirection on an alternative url without considering url parameter : [{}]", registeredService.getProperties().get("redirectLogoutURL").value());
                            logoutResponse.logoutRedirectUrl(Optional.of(registeredService.getProperties().get("redirectLogoutURL").value()));
                        } else {
