    ...
    public Map<String, List<Object>> getPersonAttributesFromMultivaluedAttributes(final Map<String, List<Object>> attributes,
        ...
        if (StringUtils.isNotBlank(username)) {
            FunctionUtils.doAndHandle(__ -> {
                ...
                if (!results.containsKey(this.usernameAttributeProvider.getUsernameAttribute())) {
                    results.put(this.usernameAttributeProvider.getUsernameAttribute(), new ArrayList<Object>(CollectionUtils.toCollection(username)));               
                }
            });
        }
        return results;
