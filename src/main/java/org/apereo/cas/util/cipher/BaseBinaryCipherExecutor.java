...
public abstract class BaseBinaryCipherExecutor extends AbstractCipherExecutor<byte[], byte[]>
    ...
    public byte[] decode(final byte[] value, final Object[] parameters) {
        try {
            val verifiedValue = this.signingEnabled ? this.verifySignature(value, this.getSigningKey()) : value;
            if (verifiedValue.length >= 28) {
                try {
                    return this.decodeWithPrefixedInitializationVector(verifiedValue);
                }
                catch (Exception e) {
                    LOGGER.trace("Unable to decrypt value using a prefixed initialization vector. CAS will attempt legacy decryption.", (Throwable)e);
                }
            }
            return this.decodeWithLegacyParameterSpec(verifiedValue);
    ...
    private byte[] decodeWithPrefixedInitializationVector(byte[] value) throws Exception {
    ...
    private byte[] decodeWithLegacyParameterSpec(byte[] value) throws Exception {
        Cipher aesCipher = Cipher.getInstance(CIPHER_ALGORITHM);
