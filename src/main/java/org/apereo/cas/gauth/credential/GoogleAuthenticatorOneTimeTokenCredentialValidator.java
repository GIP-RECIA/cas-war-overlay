...
public class GoogleAuthenticatorOneTimeTokenCredentialValidator implements
    ...
    /*
     * Customisation : new method to validate token but without scratch codes (for registration)
     */
    @Override
    public boolean isTokenAuthorizedForRegistration(final int token, final OneTimeTokenAccount account) {
        LOGGER.debug("Authorizing token [{}] against account [{}] for registration", token, account);
        val authorized = googleAuthenticatorInstance.authorize(account.getSecretKey(), token);
        return authorized;
    }
