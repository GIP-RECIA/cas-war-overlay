...
public class RedisGoogleAuthenticatorTokenCredentialRepository extends BaseGoogleAuthenticatorTokenCredentialRepository {
    ...
    public OneTimeTokenAccount update(final OneTimeTokenAccount account) {
        ...
        // Customisation : fix device duplication when using scratch code
        val redisPrincipalKey = RedisCompositeKey.forPrincipals().withPrincipal(encodedAccount).toKeyPattern();
        val accounts = casRedisTemplates.getPrincipalsRedisTemplate().boundSetOps(redisPrincipalKey).members();
        if(accounts != null){
            for(OneTimeTokenAccount oneTimeTokenAccount : accounts){
                if(oneTimeTokenAccount.getId() == account.getId() && oneTimeTokenAccount.getUsername().equals(account.getUsername())){
                    LOGGER.debug("Delete account {} from principal as it will be added back", oneTimeTokenAccount.getId());
                    casRedisTemplates.getPrincipalsRedisTemplate().boundSetOps(redisPrincipalKey).remove(oneTimeTokenAccount);
                }
            }
        }
