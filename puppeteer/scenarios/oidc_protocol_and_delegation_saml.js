...
(async () => {
    ...
    try {
        ...
        await page.goto("http://localhost:8042/oidc/protected");
        ...
        assert(pageContent.includes("\"aud\":\"client6-testcas\""));
        assert(pageContent.includes("\"iss\":\"https://localhost:8443/cas/oidc\""));
        assert(pageContent.includes("\"sub\":\"F7abc\""));

        // Call userinfo OIDC endpoint to retrieve user attributes
        await page.goto("http://localhost:8042/oidc/userinfo");
        ...
        assert(pageContent.includes("\"nickname\":\"test7\"")); //Mapped claim from ENTPersonLogin
        assert(pageContent.includes("\"uid\":[\"F7abc\"]"));
